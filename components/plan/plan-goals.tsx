...
export function PlanGoals({ goals, onAdd, onUpdate, onDelete, onAddMilestone, onToggleMilestone }: PlanGoalsProps) {
        ...
        <Card>
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium flex items-center justify-between">
                    <span className="flex items-center gap-2"><Target className="w-4 h-4" /> Goals</span>
            ...
            <CardContent>
                {goals.length === 0 ? (
                    <p className="text-sm text-muted-foreground">No goals yet. Set a goal to get started!</p>
                ) : (
                                    ...
                                    <AccordionContent className="space-y-3 pt-2">
                                            ...
                                            {goal.milestones.map(ms => (
                                                <div key={ms.id} className="flex items-center gap-2">
                                                    <Checkbox
                                                        ...
                                                        onCheckedChange={() => onToggleMilestone(goal.id, ms.id)}
                                                    />
                                        ...
                                        <div className="flex gap-2">
                                            <Input
                                                ...
                                                onChange={(e) => setNewMilestones(prev => ({ ...prev, [goal.id]: e.target.value }))}
                                                onKeyDown={(e) => e.key === "Enter" && handleAddMilestone(goal.id)}
                                                placeholder="Add milestone..."
                                                className="h-8 text-sm flex-1"
                                            />
                                            <Button size="sm" variant="outline" onClick={() => handleAddMilestone(goal.id)}>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label>Title</Label>
                            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Learn React" />
                        </div>
                        <div className="space-y-2">
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleAdd} disabled={!title.trim()}>Create Goal</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </Card>
