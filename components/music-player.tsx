...
export function MusicPlayer({
  ...
  useEffect(() => {
    if (isYTReady && window.YT && !musicPlayerRef.current) {
      try {
          ...
          events: {
            ...
            onStateChange: (event: any) => {
              try {
                ...
                console.warn("Error handling music player state change:", error)
              }
            },
            onError: (event: any) => {
    ...
    if (musicPlayerRef.current) {
      try {
        if (typeof musicPlayerRef.current.setVolume === "function") {
          musicPlayerRef.current.setVolume(musicPlayer.isMuted ? 0 : musicPlayer.volume)
        }
      } catch (error) {
  ...
  useEffect(() => {
    if ((window as any).musicPlayerControls) {
      ;(window as any).musicPlayerControls.isPlaying = () => musicPlayer.isPlaying
    }
    ...
    if (musicPlayerRef.current && musicPlayer.currentTrack) {
      try {
        if (
          ...
          typeof musicPlayerRef.current.playVideo === "function"
        ) {
          musicPlayerRef.current.loadVideoById(musicPlayer.currentTrack.videoId)
  ...
  const playNextTrack = () => {
    if (musicPlayer.playlist.length > 0) {
      ...
      if (nextIndex >= musicPlayer.playlist.length) {
        nextIndex = musicPlayer.repeat ? 0 : musicPlayer.playlist.length - 1
      }
      ...
      if (musicPlayerRef.current && nextTrack) {
        try {
          if (typeof musicPlayerRef.current.loadVideoById === "function") {
            musicPlayerRef.current.loadVideoById(nextTrack.videoId)
            if (musicPlayer.isPlaying && typeof musicPlayerRef.current.playVideo === "function") {
              musicPlayerRef.current.playVideo()
            }
          }
        } catch (error) {
  ...
  const playPreviousTrack = () => {
    if (musicPlayer.playlist.length > 0) {
      ...
      if (prevIndex < 0) {
        prevIndex = musicPlayer.repeat ? musicPlayer.playlist.length - 1 : 0
      }
    ...
    <Card className="overflow-hidden">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={onToggleQueue} className="h-8 w-8 p-0">
            {showMusicQueue ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
          </Button>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4 pb-3">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <div className="text-center mb-3">
            <h3 className="font-medium mb-1 truncate">{musicPlayer.currentTrack?.title || "No track selected"}</h3>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mb-3">
            ...
            <Button
              onClick={musicPlayer.isPlaying ? pauseMusic : playMusic}
              size="sm"
          ...
          <div className="flex items-center gap-2">
            <Button
              onClick={() => onUpdateMusicPlayer({ isMuted: !musicPlayer.isMuted })}
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0"
            >
              {musicPlayer.isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
            </Button>

            <Slider
              value={[musicPlayer.volume]}
              onValueChange={([value]) => onUpdateMusicPlayer({ volume: value })}
              max={100}
              min={0}
              className="flex-1"
            />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => onUpdateMusicPlayer({ shuffle: !musicPlayer.shuffle })}
                  variant={musicPlayer.shuffle ? "default" : "outline"}
                  size="sm"
                  className="h-8 w-8 p-0"
                >
                ...
                <Button
                  onClick={() => onUpdateMusicPlayer({ repeat: !musicPlayer.repeat })}
                  variant={musicPlayer.repeat ? "default" : "outline"}
                  size="sm"
                  className="h-8 w-8 p-0"
                >
            ...
            <div className="flex gap-2">
              <Input
                value={newMusicUrl}
                onChange={(e) => onSetNewMusicUrl(e.target.value)}
                placeholder="YouTube URL"
                size={1}
              />
              <Button onClick={handleAddMusicTrack} size="sm" className="shrink-0">
            ...
            {musicPlayer.playlist.length > 0 ? (
                ...
                {musicPlayer.playlist.map((track, index) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded-md border ${
                      musicPlayer.currentIndex === index ? "bg-blue-50 border-blue-200" : "hover:bg-gray-50"
                    }`}
                  >
                    ...
                    <div className="flex gap-1 shrink-0">
                      <Button
                        ...
                        size="sm"
                        variant={musicPlayer.currentIndex === index ? "default" : "ghost"}
              ...
              <p className="text-sm text-gray-500 text-center py-2">No tracks in queue</p>
            )}
          </div>
        )}
      </CardContent>
